...
U         = conv(U,input);
U         = U(1:N);
V         = 32;   %!!!! precision on observation noise
SCKS      = spm_DEM_generate(M,U,{P},{V,32},{32});
spm_DEM_qU(SCKS.pU)
...
SCKS.M(2).V  = 50*nD;   % inpud noise precison (fixed)
SCKS.M(1).V  = SCKS.M(1).V +exp(V);  
SCKS.M(1).xP = eye(4)*1e-10^2*nD;   % state error covariance matrix
SCKS.M(1).uP = eye(1)*1e-5^2*nD;   % input error covariance matrix
SCKS.M(1).wP = eye(np)*1e-5^2*nD;  % parameter error covariance matrix
...
[SCKS] = spm_CKS2new(SCKS0);      % NON-AUGMENTED STATE SCKS
