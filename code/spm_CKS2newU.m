function [SCKS]  = spm_CKS2new(SCKS,fig)
% FORMAT SCKS  = spm_SCKS2new(SCKS,fig)
...
xx(:,1) = xc;
Sc      = cell(1,T);
[Sc{:}] = deal(sparse(nx+nw,nx+nw));
Sc{1}   = blkdiag(Sx,Sw);

% get vector indices for components of concatenated state vector
xmask = [ones(1,nx),ones(1,nw)*3,ones(1,no)*4];
xind  = find(xmask==1);
wind  = find(xmask==3);
clear xmask;
...
%--------------------------------------------------------------------------
n          = nx + nw;       % total state vector dimension
...
if ~M(1).nograph
    ...
    f2 = spm_figure('Create','Graphics','SCKF-SCKS estimates2');
...
try,  ap     = M(1).E.RMw;     catch,  ap     = [1e3 1e8]; end   % Robins-Monro approximation paramters
try,  ap2     = M(1).E.RMq;    catch,  ap2     = [1e2 1e8]; end   % Robins-Monro approximation paramters
MLdiff0  = 0;
mloglik0 = 0;
ML       = [];
VBrun    = RUN;
...
for run = 1:RUN
    ...
    % ==================================================================
     for t = 2:T
       % sQ(2:end,2:end) = diag(diag((1/sqrt(0.9995)-1)*Sc{t-1}(xind(2:end),xind(2:end))));
        ...
        %------------------------------------------------------------------
        f             = M(1).f(Xi(xind,:),u(t-1),pE);
        % integration by local-linearization scheme:
        %------------------------------------------------------------------
        dfdx          = spm_diff_all(M(1).f,Xi(xind,:),u(t-1),pE,1);
        ...
        [ill,S]         = qr([X blkdiag(sQ,sW)]',0);
        S             = S';
        
        % in the case of VB observation noise estimation:
        %---------------------------------------------------------
        if ~isempty(M(1).Q) && iter~=1
            alpha     = lambda.*alpha + 1/sqrt(nD);  % for measurement noise covariance  !!!! Here you can play with the value between 0.2-1
            beta      = lambda.*beta;          %                              
            beta0     = beta;
        end
        ...
        %------------------------------------------------------------------
         if ~isempty(ip)
             ...
             sW    = diag(dv);
             ap(1) = min(ap(1)+0.001,ap(2));
         end
             subKG = K(xind,:);
             dq    = sqrt((1-1/ap2(1))*(dq.^2) + 1/ap2(1)*diag(subKG*(subKG*resid*resid')'));
             sQ(2:end,2:end) = diag(dq(2:end));
    ...
    % END of forward pass
    % ---------------------------------------------------------------------
    % anneal lambda over iterations (VB part)
    if ~isempty(M(1).Q) && iter~=1  
    ...
    for t = T-1:-1:1
        ...
        [ill,S]    = qr([X01 - K*Xf{t}, K*blkdiag(sQ,sW),K*Sc{t+1}]',0);
        S        = S';
        Sc{t}    = S;
    ...
    %----------------------------------------------------------------------
    figure(f2)
    s1=subplot(3,2,1:2); plot(abs(SS')); title('Standard deviation on parameter estimate')
    FIM = (mean(xx(wind,:),2).^2)./(diag(Sc{T}(wind,wind)*Sc{T}(wind,wind)').^2).*diag(sW*sW');
    %D = eig(FIM);
    subplot(3,2,3:4), bar([1:nw],diag(FIM));   
    s4 = subplot(3,2,5); plot(yy(:,2:end)'-mean(yy(:,2:end))',y(:,2:end)'-mean(y(:,2:end))','.'); axis(s4,'square');
    s5 = subplot(3,2,6); plot([yy(:,2:end)',y(:,2:end)',yy(:,2:end)'-y(:,2:end)']);     
    ...
    if RUN>1 %&& (~isempty(ip) || ~isempty(M(1).Q))
        ...
        if ( (abs(MLdiff(run)/MLdiff0)<Itol || run==RUN) || ...
            ...
            SCKS.qU.x{2} = XXf(xind,1:1:end);
            SCKS.qU.x{1} = XXb(xind,1:1:end);
         %   SCKS.qU.v{2} = XXf(uind,1:1:end);
         %   SCKS.qU.v{1} = XXb(uind,1:1:end);
            SCKS.qU.r{1} = yy;
            SCKS.qU.z{1} = res;
            SCKS.qU.rr{1} = RRR;
             SCKS.qU.ww{1} = WW;
              SCKS.qU.ss{1} = SS;
            if ~isempty(ip)
                SCKS.qP.P{1} = XXb(wind,1:1:end);
                SCKS.qP.P{2} = XXf(wind,1:1:end);
            end
            SCKS.F       = ML;
