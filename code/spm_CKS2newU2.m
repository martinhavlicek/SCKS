...
try
    if M(2).voff == 1   % flag to switch off input
        sV    = [];
        u     = [];
        Su    = [];
    end
...
for run = 1:RUN
    ...
    % ==================================================================
     for t = 2:T
       ...
       %Sc{t-1}(wind,wind) = Sc{t-1}(wind,wind)*(1/sqrt(0.9997));
        ...
        xPred(uind,:) = xPred(uind,:) + 0.5*pE(8,:).*U(:,t*OnesNpts);
        %------------------------------------------------------------------
        ...
        if ~isempty(M(1).Q) && iter~=1
            alpha     = lambda.*alpha + 1;%/N; % this my modification for this particular case 
            beta      = lambda.*beta;         % otherwise it should be + 1
            beta0     = beta;
        end
    ...
    if RUN>1 %&& (~isempty(ip) || ~isempty(M(1).Q))
        ...
        mloglik0 = mloglik;
        xc       = [xx([xind,uind],2); mean(xx(wind,:),2)];
        xx(:,1)  = xc;
        Sc{1}    = Sc{end};
