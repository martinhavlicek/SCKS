function [SCKS]  = spm_SCKS2(SCKS,fig)
...
if ~isempty(M(2).v)
    sV    = sparse(real(spm_sqrtm(inv(M(2).V)))*(dt));       % input noise variance
else
    sV    = [];
end
...
if ~isempty(M(2).v)
    Su = sparse(real(spm_sqrtm(M(1).uP))*(dt));
else
    Su = [];
end
if ~isempty(ip)
...
for run = 1:RUN
    ...
    % ==================================================================
     for t = 2:T
        ...
        Xi =  xc(:,ones(1,nPts)) + Sc{t-1}*CubPtArray;
        %------------------------------------------------------------------
        % PREDICTION STEP:
        %------------------------------------------------------------------
        xPred(uind,:) = Xi(uind,:);% 
        ...
        x10  = x1;
        Xi            = x1(:,ones(1,nPts)) + S*CubPtArray;
        X             = (Xi-x1(:,ones(1,nPts)))/sqrt(nPts);
        pE(ip,:)      = Xi(wind,:);
        ...
        for it = 1:iter
           % VB part - update of square-root measurement noise cov
            if ~isempty(M(1).Q) && iter~=1
                sR{t} = diag(diag(spm_sqrtm(1/(NU-no-1)*V))); 
            end
            ...
            % VB part:
            if ~isempty(M(1).Q) && iter~=1
                Xi       = repmat(xc,1,nPts) + S*CubPtArray;
                pE(ip,:) = Xi(wind,:);
    ...
    for t = T-1:-1:1
        ...
        Xi =  xx(:,t*ones(1,nPts)) + Sc{t}*CubPtArray;
        %
        xPred(uind,:) = Xi(uind,:);
        xPred(wind,:) = Xi(wind,:);
        ...
        [~,S]  = qr([X01 - K*X, K*blkdiag(sQ,sV,sW),K*Sc{t+1}]',0);
        S        = S';
        Sc{t}    = S;
